...
#define WaveDump_Release        "3.7.2_20160420"
#define WaveDump_Release_Date   "Apr 2016"
#define DBG_TIME
...
int WriteOutputFilesx742_Caltech(char outputFilename[], WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, CAEN_DGTZ_X742_EVENT_t *Event)
{
  printf("\nSTARTING WRITING TO FILE\n");
	...
	if (!WDrun->fout[0]) 
	  {
	    sprintf(fname, "%s", outputFilename);
	    printf("Writing to output file : %s",fname);
	    flag = 0;
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3) {
      ...
      printf("Run using wavedump [ConfigFile] [OutputFile]\n");
      return 0;
    } else {
...
Restart:
    ...
    //Enable Continuous Write Mode
    WDrun.ContinuousWrite ^= 1;
    if (WDrun.ContinuousWrite) {
      printf("Continuous writing is enabled\n");
    }
    
    //Si: here we should try to run the sequence that happens after pressing "s" on keyboard
    /* if (WDrun.AcqRun == 0) { */
    /*   printf("started\n"); */
    /*   // Avoid calibration for X731 (it is done automatically when */
    /*   // switching DES mode enablement. */
    ...
    //Set up the run state file
    //at start it should be in start state. If not, then complain about it.
    char runstateString[50];
    FILE * runstatefile;
    runstatefile = fopen("/tmp/RunFile.test.txt","r");
    if (runstatefile) {
      fgets(runstateString,50,runstatefile);
      trimTrailing(runstateString);
      //printf( "Read %s\n", runstateString);
      //printf( "test %i\n",strcmp(runstateString,"start"));
      if (strcmp(runstateString,"start") == 0) {
    	printf("Starting Run %i\n",0);
    	printf("Acquisition started\n");
    	CAEN_DGTZ_SWStartAcquisition(handle); 
    	WDrun.AcqRun = 1;
      } else {
    	printf ("Run state is not in start condition. Something is wrong. Debug the program...\n");
    	return 0;
      }      
      fclose(runstatefile);  
    } else {
      printf ("Run state file not found. Something is wrong. Debug the program...\n");
      return 0;
    }
	...
	if (WDrun.ContinuousWrite || WDrun.SingleWrite) {
	  ...
	  if (BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE) {
	    ...
	    ret = WriteOutputFilesx742_Caltech(OutputFileName, &WDcfg, &WDrun, &EventInfo, Event742);
	  }
	...
	//Check the run state file to see if we should stop data acquisition
	runstatefile = fopen("/tmp/RunFile.test.txt","r");
	if (runstatefile) {
	  fgets(runstateString,50,runstatefile);
	  trimTrailing(runstateString);
	  printf( "Read %s\n", runstateString);
	  printf( "test %i\n",strcmp(runstateString,"stop"));    	
	  if (strcmp(runstateString,"stop") == 0) {
	    printf("Acquisition stopped\n");
	    CAEN_DGTZ_SWStopAcquisition(handle);
	    WDrun.AcqRun = 0;
	    exitRunLoop = 1;
	    printf("Detected stop signal. Stopping Run now.\n");
	  }
	} else {
	  printf ("Run state file not found. Something is wrong. Debug the program...\n");
	}
    ...
    printf("Stopped Run %i. Collected Total of %i\n", 0,NTriggers);
