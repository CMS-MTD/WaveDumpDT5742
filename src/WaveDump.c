...
Restart:
    ...
    //By Default enable Continuous Write Mode
    WDrun.ContinuousWrite ^= 1;
    if (WDrun.ContinuousWrite) {
      printf("Continuous writing is enabled\n");
    }
    ...
    //File to check for OTSDAQ commands
   ...
   /* *************************************************************************************** */
    ...
    while(!WDrun.Quit) {
        ...
        //check if OTSDAQ is issuing a run stop signal
        runstatefile = fopen("/tmp/RunFile.txt","r");
	if (runstatefile) {
	  ...
	  if (strcmp(runstateString,"stop") == 0) {	  
	    printf("Detected stop signal from OTSDAQ. Stopping Run now.\n");
	    WDrun.Quit = 1;  	
	  }
	} else {
...
QuitProgram:
    ...
    printf("Total Number of Events Saved: %i\n",NumberOfEvents);
